...
def save_retailer_info(rtlr_info):
	global col
	connect_db()
	col.insert(rtlr_info)
...
# search by id
def search_installer_by_pName(prgrmName):
   global col_installer
   connect_db()
   searched_doc = col.find({'ProgramName':str(prgrmName)})
   return searched_doc